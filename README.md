# Микросервисы маркетплейса — учебный стенд для QA

Учебный проект с микросервисной архитектурой: маркетплейс из трёх компонентов (фронт, сервис заказов, сервис оплаты) с RabbitMQ, Kafka, PostgreSQL, Loki, Tempo и Grafana.

На этом стенде можно пройти путь одного заказа целиком — от API и очередей до топиков, логов и трейсов по **traceId**.

Репозиторий подойдёт, если хочется руками посмотреть, как выглядит тестовое окружение для QA на микросервисном проекте, и потрогать базовые сценарии, которые часто остаются «где-то на словах».
...
- код сервисов
- `docker-compose.yml` и конфигурация окружения
- всё необходимое для локального запуска стенда
...
- разборы сценариев
- словарь терминов
- практические материалы по тестированию этого стенда

То есть сам стенд открыт, но обучающая часть выдаётся отдельно.

## Для чего можно использовать этот проект

На стенде можно потренироваться:

- запускать микросервисное окружение локально
- отслеживать путь заказа через RabbitMQ и Kafka
- искать **traceId** в логах и трейсах
- смотреть статусы заказа в базе
- лучше понимать, как QA работает не только с UI, но и с системой целиком

## Если хочется пройти это не вслепую

Если нужен не просто репозиторий, а пошаговый разбор: что именно здесь проверять, куда смотреть, как мыслить в таких сценариях и как это связано с реальной работой QA — это есть в рамках моего интенсива по тестированию микросервисной архитектуры.

- **Курс на Stepik:** *https://stepik.org/a/275445*  
- **Связаться в Telegram:** *https://t.me/svyaushev*
